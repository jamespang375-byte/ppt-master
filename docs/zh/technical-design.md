...
**为什么真正的硬约束留在上游。** 跨切的技术硬约束（`<clipPath>` 只能用在 `<image>` 上、禁 `<mask>`、受支持的 CSS 颜色语法、alpha 效果的路由表）独家住在 [`shared-standards.md`](../../skills/ppt-master/references/shared-standards.md)。版式词表只用一行指针指向它们，不复述——这样某条约束放开时（比如某个 DrawingML 特性变得可靠），只有一个文件要改，词表里也不会留下一份过期副本继续暗中强制旧规则。

---

## SVG 约束：禁用特性与条件允许

PowerPoint 的 DrawingML 是 SVG 表达力的严格子集。Executor 在一份经验生长起来的黑名单（mask、style/class、`@font-face`、foreignObject、textPath、animate*、script/iframe ……）里运行，外加对 `marker-start`/`marker-end`、仅 `<image>` 上的 `clip-path` 和同文档静态 `<use>` / `<symbol>` 复用的窄条件允许。权威清单和每条特性的具体约束——包括 `<mask>` 的替代效果路由表（渐变叠加、clipPath、filter shadow、源图烘焙）——住在 [`references/shared-standards.md`](../../skills/ppt-master/references/shared-standards.md)。

**为什么本地 `<use>` 是编译期复用，不是 PowerPoint 保留对象。** 允许的引用必须是精确的同文档 `href="#id"` 或 `xlink:href="#id"`，目标仅限 `symbol`、`g`、`use`、`rect`、`circle`、`ellipse`、`line`、`path`、`polygon`、`polyline`、`text`、`image`，内部引用使用精确的 `url(#id)` 片段。`x` / `y` 只能是有限 unitless 或 `px` 值。`symbol` 必须有合法且维度为正数的 `viewBox`，实例必须提供正数 unitless/`px` `width` / `height`，并使用对齐的 `meet` 或纯 `none`；`slice`、`refX`、`refY` 会被拒绝。由于展开不会复现 overflow clipping，symbol 图形必须留在 `viewBox` 内。外部、缺失、冲突、循环、重复 ID 引用，以及对 `data-pptx-layer*`、`data-pptx-native*`、`data-pptx-placeholder*` 结构的复用都会校验失败。流水线会递归实体化子树并重写克隆 ID 后再导出；PPTX 回导只返回展开后的原语，不重建创作期 `<use>` 图。
...
**写盘消费者** —— `finalize_svg.py` 每次运行都把 `svg_output/` → `svg_final/` 写到磁盘一次，同时展开项目图标占位符和合规的本地 `<use>` 引用。`svg_final/` 随后供 IDE 预览和 preview pptx 使用。
