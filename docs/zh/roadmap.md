...
> **已支持边界**：`template-fill-pptx` 直接回填选中的源页面；`create-template` 把源 Master/Layout 事实收敛成经过确认、完整且显式分层的 SVG 模板。下游严格与适应性使用都从 SVG 契约还原原生结构。仍不做未经审查、没有契约的任意第三方 placeholder 全自动替换。
