...
When the user says "豆包风格" / "doubao style", load the brand template at [`skills/ppt-master/templates/brand-doubao-huawei/design_spec.md`](skills/ppt-master/templates/brand-doubao-huawei/design_spec.md). This is a `kind: brand` template — copy it into the project's `templates/` directory before Step 4 so the Strategist locks identity from it. Key rules: 浅色系背景（封面不用深蓝）、华为红标题(#C00000)、直角无圆角、扁平无阴影、左侧5px彩条卡片、标题下彩色短线、三色协奏(#C00000+#D4A843+#003366)、顶部红色细线、右下角华为Logo(可选)。

**信息密度是豆包风格的硬约束** — 品牌模板定义了每页文本量下限（内容页≥800字/≥40元素）、每张卡片的5层信息结构（标签→数据→解释→对比值→来源）、区域填充规则和元素复用手法。

**图片配图是豆包风格的硬约束** — 当 `.env` 配置了 `IMAGE_BACKEND` 和 `PEXELS_API_KEY` 时，Strategist 必须将 `image_usage` 设为 `["ai", "web"]`。封面用AI生图低透明度叠加（浅色背景上），收尾用AI生图+深蓝遮罩，至少2页内容页嵌入 Pexels 实景图片。
